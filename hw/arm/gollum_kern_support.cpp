...
uint32_t encode_bl(uintptr_t source, uintptr_t target) {
    ...
    uint64_t bl_inst_diff_imm = (target - source) >> 2;
...
craft_shellcode(AddressSpace *nsas, MemoryRegion *mem, uintptr_t *in_out_curr_pa, uintptr_t hook_addr,
    ...
    *in_out_curr_pa += alloc_size;
    ...
    uint32_t bl_shellcode = encode_bl(hook_addr, shellcode_start_addr);
    address_space_rw(nsas, vtop_static(hook_addr), MEMTXATTRS_UNSPECIFIED, (uint8_t *) &bl_shellcode, 4, 1);
